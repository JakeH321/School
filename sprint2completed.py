            ...
            while play == 1:  # this just makes it so that you don't need to keep running the code.
                while True:  # This allows the question to be repeated on improper input rather than breaking.
                    try:
                        ...
                        ComparisonA = 1
                        ComparisonB = 0
                        Truth = not ComparisonA > ComparisonB
                        if not Truth:
                        ...
                        try:
                            play = int(input(" Would you like to continue playing? Press anything but 2 for yes or 2 "
                                             "for no."
                                             " "))
                        except ValueError:
                    ...
                    # much aside from fulfill the function requirement.
                    # uses the definition count =total_runs in def main()
                    # then is run when main() is run
                    def run_count(count):
                        """" Uses the parameter count
                        """
                        print("You only need", count, "more runs until a milestone,congratulations")

                    # uses the parameter of count, and sets it equal to "runs"
                    # this isn't any different in
                    # execution from just printing out runs
                    # it is just for the function requirement.
                    def main():
                        """" uses the parameter of count, and sets it equal to "runs"
                        """
